...
def maneuver_forward_backward(car):
    car.stop()
    car.forward(50)
    time.sleep(2)
    car.backward(50)
...
def maneuver_parallel_parking(car, left=True):
    sign = 1 if not left else -1
    car.set_dir_servo_angle(sign * 45)
    time.sleep(0.5)
    car.backward(20)
    time.sleep(1)
...
def maneuver_three_point_turn(car, left=True):
    sign = 1 if not left else -1
    car.set_dir_servo_angle(sign * 45)
    car.forward(20)
    time.sleep(1)
...
if __name__ == "__main__":
    ...
    while True:
        choice = input("Enter 'q', '1' for forward-backward, '2' for parallel parking, '3' for a 3-point turn: ")

        if choice == 'q':
            break
        if choice == '1':
            maneuver_forward_backward(px)
        if choice == '2':
            direction = None
            while direction is None:
                ...
                if direction not in ['l', 'r']:
                    ...
                    left = True if direction == 'l' else False
            maneuver_parallel_parking(px, left)
        if choice == '3':
            direction = None
            while direction is None:
                direction = input("Choose left ('l') or right ('r'): ")
                if direction not in ['l', 'r']:
                    ...
                    left = True if direction == 'l' else False
            maneuver_three_point_turn(px, left)

    #maneuver_parallel_parking(px, left=True)
    #maneuver_parallel_parking(px, left=False)
    #px.forward(50)
    #time.sleep(5)
    #px.stop()
